...
| **[skill-manager](packages/skill-manager)** | Вкладка **«Скиллы»** в Настройки → Плагины, после «Модов»: всё, что этот Harness видит, с описанием, по которому сопоставляется запрос, и краткой строкой для человека. Поставил на паузу — и модель перестаёт получать этот скилл при следующем запросе, без перезапуска. |
...
русская локализация: сканирование основных каталогов DSH показало, что это
пустая ниша.
...
корень у DSH (семь мест с рангами) и какие бывают у других —
