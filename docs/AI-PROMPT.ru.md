# Промпт для ИИ — создать мод для DSH
...
Ты добавляешь плагин («мод») в **DeepSeek Harness (DSH)** — харнесс агентов,
...
- **Никогда не правь установку DSH.** Моды — отдельные пакеты, которые ставятся в
...
1. **Юнит-тесты половин** без DSH: выполни браузерный бандл с заглушкой
...
1. **Правка загруженного пакета его не перезагружает.** Загрузчик импортирует
   ...
   *Новая* строка применяется на лету; *изменённый* файл пакета требует
   перезапуска `dsh web`.
2. **Перезагрузка патча не пересоздаёт существующую строку.** Подмена `name` на
   заведомо несуществующий спецификатор оставила старый роут отвечать `200` —
   доказательство, что смонтированные записи не переимпортируются. Не планируй
   горячую подмену.
3. **`renderPrompt` падает на нерешённой `{{переменной}}`**, а синтаксиса
   экранирования в DSH нет. Если пользователь может вводить текст промпта —
...
11. **Версионный дрейф реальность.** DSH до 1.0, его плагинные API меняются.
...
Отчитывайся честно: что работает, что не проверено и что зависит от версии DSH.
