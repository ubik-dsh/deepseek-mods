...
**Эта копия живёт по адресу <https://gitverse.ru/ubikon/dsh-mods>** (на момент
написания — приватная). Команды ниже намеренно обобщённые: `<пользователь>` и
`<хост>` оставлены заглушками, чтобы инструкция годилась и для GitHub, и для
GitVerse, и для любого другого хостинга, и для того, кто публикует форк.
