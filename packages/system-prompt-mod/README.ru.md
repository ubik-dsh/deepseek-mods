...
Плагин для **харнесса**, который добавляет в шапку чата кнопку **«Промпт»**: она
...
синтаксиса экранирования в харнессе нет. Поэтому текст с группой `{{…}}`
...
Первому нужна видимость пакетов харнесса — её создаёт `node tools/dev/link-dsh.mjs`.
...
- Пуш-событий нет: окно читает состояние при открытии, потому что харнесс не
