...
## Поставить найденное — это отдельный шаг, а не «скопировал и надеюсь»

**Найденный скилл не считается принятым, пока он не смонтирован и не виден агентам.** Порядок:

1. **Прочитать кандидата целиком** до установки — `SKILL.md` и то, что он подтягивает.
2. **Скопировать в источник семьи** (`deepseek-harness-skills/skills/<имя>`), а не в дерево, которое
   живёт до первой синхронизации.
3. **Синхронизировать:** `node tools/dev/check-skills-synced.mjs --sync` в `dsh-mods`.
4. **Проверить, что он появился у агентов** — по каталогу скиллов, а не по факту копирования:
   файл на диске и скилл в каталоге — разные вещи.
5. **Записать источник** — откуда взято и на какой проверке остановились.

**Взято у их `skill-installer`** (установка прямо по пути в GitHub): у нас есть поиск
(`find-a-skill`) и есть раскладка по трём деревьям, а **шага установки с проверкой не было.**
