...
const MESSAGES = {
  en: {
    ...
    done: 'Done. Reload the Web GUI (F5). If a mod you just installed does not appear, restart `dsh web`: a loader row whose import failed once stays failed until then.',
  ...
  ru: {
    ...
    done: 'Готово. Обнови страницу Web-GUI (F5). Если только что установленный мод не появился, перезапусти `dsh web`: строка загрузчика, чей импорт однажды упал, остаётся упавшей до этого.',
