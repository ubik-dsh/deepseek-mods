...
const MESSAGES = {
  en: {
    ...
    done: 'Done. Reload the Web GUI (F5). A mod that was already installed keeps serving the build it loaded at boot, so restart `dsh web` after an update — and if a newly installed mod does not appear at all.',
  ...
  ru: {
    ...
    done: 'Готово. Обнови страницу Web-GUI (F5). Мод, который уже был установлен, продолжает отдавать сборку, загруженную при запуске, поэтому после обновления перезапусти `dsh web` — а также если новый мод вообще не появился.',
